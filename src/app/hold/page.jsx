...
import { getHoldData, getHoldProductData, getNavLinkMenu, otherCategoriesData } from "../service/apiService";

export default async function Page() {
    const navItems = await getNavLinkMenu();
    const holdData = await getHoldData();
    const otherCategories = await otherCategoriesData()
    const holdProductData = await getHoldProductData();
    console.log("holdProductData", holdProductData)

    if (!holdData || !navItems || !otherCategories || !holdProductData) return null;
    const {
...
<img src={`${process.env.NEXT_PUBLIC_API_URL}${image?.url}`}
  ...
  {/* Product Grid */}
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 p-4">
                    {
                        holdProductData && holdProductData?.length > 0 && holdProductData?.map((product) => (
                            <a key={product?.id}
                            href={product?.completeurl || "#"}
                             className="flex flex-col items-center justify-center gap-y-4">
                                <img src={`${process.env.NEXT_PUBLIC_API_URL}${product?.image?.url}`}
